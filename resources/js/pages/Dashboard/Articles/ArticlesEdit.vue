...
const form = useForm({
  ...
  category_id: props.article?.categories[0],
...
const submit = async () => {
  if (!form.category_id) {
    form.errors.category_id = 'Kategori wajib dipilih'
    return
  }
  if (!form.tag_ids || form.tag_ids.length === 0) {
    form.errors.tag_ids = 'Minimal 1 tag wajib dipilih'
    return
  }
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    ...
    <form @submit.prevent="submit" class="p-4 space-y-6">
      ...
      <div>
        ...
        <multiselect v-model="form.category_id" :options="cat" :multiple="false" :group-select="true"
          :allow-empty="required" placeholder="Pilih kategori artikel" track-by="name" :label="`name`"
          :searchable="false">
      ...
      <div>
        ...
        <QuillEditor v-model:content="form.body" theme="snow" toolbar="full" contentType="html" :modules="modules"
          :BlotFormatter="BlotFormatter" style="height: 400px;" />
    ...
    <form @submit.prevent="UploadArticleCover" class="p-4 pt-10">
      ...
      <div class="w-[400px] border-2 border-gray-300 border-dashed rounded-lg p-6" id="dropzone"
        ...
        <!-- Preview Cover -->
        <img v-if="!previewUrl" :src="`/storage/${props.article?.cover}`" class="mt-4 mx-auto max-h-40"
          alt="Cover Saat Ini" />
        <img v-if="previewUrl" :src="previewUrl" class="mt-4 mx-auto max-h-40" alt="Preview Cover Baru" />
      </div>
