            ...
            <PopoverPanel
              ...
              <div class="p-8 grid grid-cols-3 gap-8">
                ...
                <div v-for="item in products" :key="item.name" class="flex gap-x-4">
                  ...
                  <div>
                    <a :href="item.href" class="block font-semibold text-gray-900 hover:text-green-600 dark:text-gray-300 dark:hover:text-green-500">
                      {{ item.name }}
                    </a>
                  ...
                  <Disclosure as="div" class="-mx-3" v-slot="{ open }">
                    <DisclosureButton
                      ...
                      <img
                        ...
                        alt="Profile Photo" class="h-8 w-8 rounded-full object-cover" />
                      <span class="text-sm font-semibold text-gray-900 dark:text-gray-200 px-2"> {{ $page.props.auth.user.name }}</span>
                      <ChevronDownIcon :class="[open ? 'rotate-180' : '', 'size-5 flex-none', 'dark:text-gray-200']" aria-hidden="true" />
                    </DisclosureButton>
                    <DisclosurePanel class="mt-2 space-y-2">
...
const products = [
  ...
  { name: 'News', description: 'Kumpulan berita terkini paling faktual', href: '#', icon: NewspaperIcon },
  { name: 'Life Style', description: 'Tips & tricks buat kamu yang mau improvisasi diri', href: '#', icon: SparklesIcon },
  { name: 'Hobby', description: 'Dari diving di lautan hingga sky diving dari langit semua ada disini', href: '#', icon: RocketLaunchIcon },
  { name: 'Review', description: 'Lihat pengalaman terseru mengenai game, software dan film!', href: '#', icon: ChatBubbleLeftRightIcon },
